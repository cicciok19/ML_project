...
def xml_to_csv(path):
    ...
    for xml_file in glob.glob(path + '/*.xml'):
        ...
        root = tree.getroot()
        for mb in root.findall('object'):
            value = (root.find('filename').text,
                     int(root.find('size')[0].text),
                     int(root.find('size')[1].text),
                     mb[0].text,
                     math.floor(float(mb[4][0].text)),
                     math.floor(float(mb[4][1].text)),
                     math.floor(float(mb[4][2].text)),
                     math.floor(float(mb[4][3].text))
                     )
...
if sys.argv[1]=='train':
    path_xml = "../dataset/train"
    output_path = "../annotations/train.record"
elif sys.argv[1]=='test':
    path_xml = "../dataset/test"
